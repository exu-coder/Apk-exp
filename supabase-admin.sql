...
-- Keeps the existing demo_admin_login(email,password) credential check.
-- Do NOT store a plaintext password in this repository.
...
create index if not exists demo_admin_sessions_token_hash_idx
  on public.demo_admin_sessions(token_hash);

create index if not exists demo_admin_sessions_expires_at_idx
  on public.demo_admin_sessions(expires_at);
...
revoke all on function public.demo_admin_validate_session(text) from public, authenticated;
